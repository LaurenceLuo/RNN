...
def run_network(model=None, X_train=None, X_label=None, X_test=None, X_test_label=None):
    ...
    try:
        model.fit(X_train, X_label,batch_size=5, nb_epoch=epochs)#, validation_split=0.05
    ...
    np.savetxt('StarlightCurves_train_8D.txt', layer_output, fmt='%s')
    ...
    np.savetxt('StarlightCurves_test_8D.txt', layer_output_after, fmt='%s')
    ...
    np.savetxt('StarlightCurves_test_labels_predict.txt', predict_labels, fmt='%s')
...
np.savetxt('StarlightCurves_test_labels_truth.txt', X_test_label, fmt='%s')
