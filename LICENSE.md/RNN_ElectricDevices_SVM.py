...
def run_network(model=None, X_train=None, X_label=None, X_test=None, X_test_label=None):
    ...
    print "Complete."
    print 'SVM Training duration (s) : ', time.time() - SVM_start_time
    ...
    print 'SVM Testing duration (s) : ', time.time() - SVM_test_start_time
    ...
    try:
        model.fit(X_train, X_label,batch_size=120, nb_epoch=epochs)#, validation_split=0.05
    except KeyboardInterrupt:
        print 'Training duration (s) : ', time.time() - global_start_time
        return model, 0
    print 'RNN Training duration (s) : ', time.time() - global_start_time
    ...
    np.savetxt('ElectricDevices_train_16D.txt', layer_output, fmt='%s')
    ...
    print 'RNN Testing duration (s) : ', time.time() - test_start_time
    ...
    np.savetxt('ElectricDevices_test_16D.txt', layer_output_after, fmt='%s')

    print "Starting SVM:"
    clf=SVC()
    RNNSVM_train_start_time=time.time()
    print "Training..."
    ...
    print "Complete."
    print 'RNN+SVM Training duration (s) : ', time.time() - RNNSVM_train_start_time

    RNNSVM_test_start_time=time.time()
    ...
    print 'RNN+SVM Testing duration (s) : ', time.time() - RNNSVM_test_start_time
    print predict_labels
    np.savetxt('ElectricDevices_test_labels_predict.txt', predict_labels, fmt='%s')
...
X_train=train_timeseries[:,:,0:train_timeseries.shape[2]]
#X_label=train_timeseries[:,:,[0]]
X_test=test_timeseries[:,:,0:train_timeseries.shape[2]]
...
#np.savetxt('ElectricDevices_test_labels_truth.txt', X_test_label, fmt='%s')
#np.savetxt('ElectricDevices_train_labels_truth.txt', X_label, fmt='%s')
...
print "Test Label Shape0: ",X_test_label.shape[0]
print "Test Label Shape1: ",X_test_label.shape[1]
