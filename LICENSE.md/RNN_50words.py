...
def build_model():
    ...
    model.add(LSTM(input_dim=layers[1], output_dim=layers[2], return_sequences=True))
    model.add(Dropout(0.2))
...
def run_network(model=None, X_train=None, X_label=None, X_test=None, X_test_label=None):
    ...
    print "Labels ground truth:",X_label
    clf_gt=SVC()
    ...
    #np.savetxt('50words_test_labels.txt', predict_labels_gt, fmt='%s')
    print "Predicted: ",predict_labels_gt
    count_1=0
    ...
    try:
        model.fit(X_train, X_label,batch_size=2, nb_epoch=epochs)#, validation_split=0.05
    except KeyboardInterrupt:
        print 'Training duration (s) : ', time.time() - global_start_time
        return model, 0
    print 'Training duration (s) : ', time.time() - global_start_time

    get_3rd_layer_output = K.function([model.layers[0].input, K.learning_phase()],[model.layers[2].output])
    ...
    np.savetxt('50words_train_50D.txt', layer_output, fmt='%s')
    
    eval_loss,eval_accuracy=model.evaluate(X_test,X_test_label)
    ...
    np.savetxt('50words_test_50D.txt', layer_output_after, fmt='%s')
    ...
    np.savetxt('50words_test_labels_predict.txt', predict_labels, fmt='%s')
...
np.savetxt('50words_test_labels_truth.txt', X_test_label, fmt='%s')
