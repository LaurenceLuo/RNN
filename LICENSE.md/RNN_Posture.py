...
def run_network(model=None, X_train=None, X_label=None, X_test=None, X_test_label=None):
    ...
    try:
        model.fit(X_train, X_label,batch_size=100, nb_epoch=epochs)#, validation_split=0.05
    ...
    np.savetxt('Posture_train_21D.txt', layer_output, fmt='%s')
    ...
    np.savetxt('Posture_test_21D.txt', layer_output_after, fmt='%s')
    ...
    np.savetxt('Posture_test_labels_predict.txt', predict_labels, fmt='%s')
...
X_train=train_timeseries[:,:,0:train_timeseries.shape[2]-1]
X_label=train_timeseries[:,:,[train_timeseries.shape[2]-1]]
print "Label Shape0: ",X_label.shape[0]
print "Label Shape1: ",X_label.shape[1]
X_test=test_timeseries[:,:,0:train_timeseries.shape[2]-1]
X_test_label=test_timeseries[:,:,[train_timeseries.shape[2]-1]]

idx_label_first=X_label[0][0]
idx_label_second=idx_label_first
idx_first=0
idx_second=0
count=0
X_label_new=[]
X_train_new=np.empty((0,3))
print "!!!!!",X_train_new
#print "!!!!!",X_train_new.shape[1]

for idx,train_data in enumerate(X_train[0]):
#print idx,train_data
    idx_label_second=X_label[0][idx]
    if idx_label_first!=idx_label_second:
        count+=1
        #print "Current label: ",idx_first
        idx_second=idx
        X_label_new.append(idx_label_first[0])
        #print idx_first,idx_second, ": ",X_train[0][idx_first:idx_second]
        #X_train_new.append(X_train[0][idx_first:idx_second].tolist())
        #print "!!!!!",X_train[0][idx_first:idx_second]
        #X_train_new = np.append(X_train_new, [X_train[0][idx_first:idx_second]],axis=0)
        idx_label_first=idx_label_second
        idx_first=idx_second
idx_second=X_train.shape[1]
#print idx_first,idx_second, ": ",X_train[0][idx_first:idx_second]
#X_train_new.append(X_train[0][idx_first:idx_second].tolist())
#X_train_new = np.append(X_train_new,[X_train[0][idx_first:idx_second]],axis=0)
X_label_new.append(idx_label_second[0])

print "Number of Labels: ",count
print "Labels: ",X_label_new
print "Shape0: ",X_train.shape[0]
print "Shape1: ",X_train.shape[1]
print "Shape2: ",X_train.shape[2]
#print "Label Shape0: ",X_label_new.shape[0]
#print "Label Shape1: ",X_label.shape[1]
np.asarray(X_train_new)
#print "Train data: ",X_train_new
#print "TRAIN Data Shape 0: ", X_train_new.shape[0]
#print "TRAIN Data Shape 1: ", X_train_new.shape[1]
#print "TRAIN Data Shape 2: ", X_train_new.shape[2]

idx_test_label_first=X_test_label[0][0]
idx_test_label_second=idx_label_first
idx_test_first=0
idx_test_second=0
count=0
X_test_label_new=[]
X_test_new=[]
for idx,test_data in enumerate(X_test[0]):
    #print idx,train_data
    idx_test_label_second=X_test_label[0][idx]
    if idx_test_label_first!=idx_test_label_second:
        count+=1
        #print "Current label: ",idx_first
        idx_test_second=idx
        X_test_label_new.append(idx_test_label_first[0])
        #print idx_first,idx_second, ": ",X_train[0][idx_first:idx_second]
        #X_test_new.append(X_test[0][idx_test_first:idx_test_second])
        idx_test_label_first=idx_test_label_second
        idx_test_first=idx_test_second
idx_test_second=X_test.shape[1]
#print idx_first,idx_second, ": ",X_train[0][idx_first:idx_second]
X_test_new.append(X_test[0][idx_test_first:idx_test_second])
X_test_label_new.append(idx_test_label_second[0])

print "Number of Labels: ",count
print "Test Labels: ",X_test_label_new
#print "Test data: ",X_test_new
...
print "TEST Data Shape 1: ", X_test.shape[1]
print "TEST Data Shape 2: ", X_test.shape[2]

X_label=np.reshape(X_label, (X_label.shape[1],1))
print "Labels: ",X_label
print "Labels Shape0: ",X_label.shape[0]
print "Labels Shape1: ",X_label.shape[1]
X_test_label=np.reshape(X_test_label, (X_test_label.shape[1],1))
#X_label=X_label[0]
#X_test_label=X_test_label[0]
#np.savetxt('Posture_test_labels_truth.txt', X_test_label, fmt='%s')
