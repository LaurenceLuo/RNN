...
def run_network(model=None, X_train=None, X_label=None, X_test=None, X_test_label=None):
    ...
    try:
        model.fit(X_train, X_label,batch_size=1, nb_epoch=epochs)#, validation_split=0.05
    ...
    np.savetxt('Sensor_train_3D.txt', layer_output, fmt='%s')
    ...
    np.savetxt('Sensor_test_3D.txt', layer_output_after, fmt='%s')
    ...
    np.savetxt('Sensor_test_labels_predict.txt', predict_labels, fmt='%s')
...
parser.add_argument('train_label_dir', help='directory with txt files')
parser.add_argument('test_dir', help='directory with txt files')
parser.add_argument('test_label_dir', help='directory with txt files')
...
test_label_filenames = os.listdir(test_label_dir)
train_timeseries = []
test_timeseries = []
X_label= []
X_test_label=[]
...
print "Reading training labels from", train_label_dir
for filename in train_label_filenames:
    ts = np.genfromtxt(os.path.join(train_label_dir, filename))
    if ts.ndim == 1:  # csv file has only one column, ie one variable
        ts = ts[:, np.newaxis]
    X_label.append(ts)
X_label = np.array(X_label)
print "TRAINING LABELS: ", X_label
...
print "Reading test labels from", test_label_dir
for filename in test_label_filenames:
    ts = np.genfromtxt(os.path.join(test_label_dir, filename))
    if ts.ndim == 1:  # csv file has only one column, ie one variable
        ts = ts[:, np.newaxis]
    X_test_label.append(ts)
X_test_label = np.array(X_test_label)
print "TEST LABELS: ", X_test_label
#num_train=round(train_timeseries.shape[0])
#num_test=round(test_timeseries.shape[0])

X_train=train_timeseries[:,:,2:train_timeseries.shape[2]-2]
print "Original Shape0: ",X_train.shape[0]
print "Original Shape1: ",X_train.shape[1]
print "Original Shape2: ",X_train.shape[2]
#X_label=train_timeseries[:,:,[0]]
print "Label Shape0: ",X_label.shape[0]
print "Label Shape1: ",X_label.shape[1]
X_test=test_timeseries[:,:,2:train_timeseries.shape[2]-2]
print "Original TEST Data Shape 0: ", X_test.shape[0]
print "Original TEST Data Shape 1: ", X_test.shape[1]
print "Original TEST Data Shape 2: ", X_test.shape[2]
...
X_train=np.reshape(X_train, (39, 3000,8))
print "TRAIN TIME SERIES: ", X_train
print "Shape0: ",X_train.shape[0]
print "Shape1: ",X_train.shape[1]
print "Shape2: ",X_train.shape[2]
X_test=np.reshape(X_test, (60, 3000,8))
